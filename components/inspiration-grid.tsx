...
import {
	inspirationData,
	type InspirationItem as ImportedInspirationItem,
...
export default function InspirationGrid({
	...
	const filteredInspirations = inspirationData.filter(
		item =>
			...
			<div className={`${getGridClass()} gap-4 w-full`}>
				{filteredInspirations.map(item => {
					...
					return (
						<Card
							key={item.title}
							className={`overflow-hidden w-full ${gridView === "list" ? "flex" : ""}`}
						>
							<CardContent
								className={`p-0 ${gridView === "list" ? "flex items-center w-full" : ""}`}
							>
								<div
									className={`relative ${gridView === "list" ? "w-24 h-24 flex-shrink-0" : ""}`}
								>
									<img
										src={item.image}
										alt={item.title}
								...
								<div
									className={`${gridView === "list" ? "flex-grow flex justify-between items-center px-2" : "p-4"}`}
								>
									<div>
									...
									<div
										className={`${gridView === "list" ? "ml-4" : "mt-4"} flex gap-2`}
									>
										<Button
											variant="secondary"
											size="sm"
